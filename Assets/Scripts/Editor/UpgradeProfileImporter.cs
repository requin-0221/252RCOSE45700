...
public class UpgradeProfileImporter : EditorWindow
{
    // CSV 파일명 접두사
    private const string CSV_PREFIX = "UpgradeStatTable_";

    // Scriptable Object에 데이터 주입
    private static string baseAssetPath = "Assets/Resources/Data/Upgrade/";

    private const int STAT_COUNT = 19;

    [MenuItem("Tools/Import Upgrade Stat Table")]
    public static void ImportStatTable()
    {
        // 프로젝트 내 모든 CSV 파일 검색
        string[] guids = AssetDatabase.FindAssets("UpgradeStatTable_ t:TextAsset");
        if (guids.Length == 0)
        {
        ...
        foreach (string guid in guids)
        {
            ...
            // 키워드 추출 (예: UpgradeStatTable_Weapon -> Weapon)
        ...
        Debug.Log($"UpgradeProfile {successCount}개 생성");
    }
    ...
    public static bool CreateUpgradeProfile(string keyword, string csvContent)
    {
        ...
        string assetFullPath = baseAssetPath + $"UpgradeProfile{keyword}.asset";
        UpgradeProfile profile = AssetDatabase.LoadAssetAtPath<UpgradeProfile>(assetFullPath);

        if (profile == null)
        {
            profile = ScriptableObject.CreateInstance<UpgradeProfile>();
